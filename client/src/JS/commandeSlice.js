...
export const getcommande=createAsyncThunk('/getcommande',async()=>{
    try {
        let resultat= await axios.get('http://localhost:5500/commande/all')
        return  resultat;
    } catch (error) {
        console.log(error)
    }
...
export const addcommande=createAsyncThunk('/addcommande',async(newcommande)=>{
    try{
        let resultat= await axios.post('http://localhost:5500/commande/add',newcommande)
        return  resultat;   
...
export const updatecommande=createAsyncThunk('/updatecommande',async(id,upcommande)=>{
    try{
        let resultat= await axios.put(`http://localhost:5500/commande/${id}`,upcommande)
        return  resultat;   
